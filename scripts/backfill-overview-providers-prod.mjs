// backfill-overview-providers-prod.mjs
// 對正式區 movies：search TMDB → 取 tmdb_id / overview / watch_providers TW link → PATCH 回正式區。
//
// 執行：
//   PROD_ADMIN_EMAIL=... PROD_ADMIN_PASSWORD=... \
//     node midnight-be/scripts/backfill-overview-providers-prod.mjs --dry-run --limit=3
//   PROD_ADMIN_EMAIL=... PROD_ADMIN_PASSWORD=... \
//     node midnight-be/scripts/backfill-overview-providers-prod.mjs --limit=50
//   PROD_ADMIN_EMAIL=... PROD_ADMIN_PASSWORD=... \
//     node midnight-be/scripts/backfill-overview-providers-prod.mjs
//   ... --refresh  # 已有 tmdb_id 的也重抓
//   ... --reset    # 清空 prod 專屬進度檔
//
// 設計與原版相同：
//   - 預設 idempotent：有 tmdb_id 的跳過（除非 --refresh）
//   - TMDB rate limit：~4 req/sec；每部 2 calls (search + watch/providers) → 配 260ms 間隔
//   - 進度檔：tmp/backfill-overview-processed-prod.txt（每筆寫一行 movie.id；與本機分開）
...
const PROGRESS_FILE = join(__dirname, '..', 'tmp', 'backfill-overview-processed-prod.txt')
const TMDB_INTERVAL_MS = 260

if (!TMDB_KEY) { console.error('❌ midnight-be/.env 缺少 TMDB_API_KEY'); process.exit(1) }
if (!EMAIL || !PASSWORD) { console.error('❌ 缺少 PROD_ADMIN_EMAIL / PROD_ADMIN_PASSWORD'); process.exit(1) }
...
// ── Main ──────────────────────────────────────────────────
async function main() {
  console.log(`🎯 target: ${BASE}${DRY_RUN ? ' (dry-run)' : ''}`)
